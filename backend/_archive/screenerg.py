...
STRATEGY_TO_RUN = 'REVERSED_SHORT'  # 新添加的策略选项
...
def check_triple_cross_enhanced_filter(df, signal_idx, stock_code):
    ...
    修改：放松涨幅判断，只排除涨幅>30%的股票，移除下限以允许更多股票通过
    """
    try:
        # 1. 使用增强版过滤器
        ...
        if signal_idx >= 5:
            price_5_days_ago = df.iloc[signal_idx - 5]['close']
            short_term_gain = (current_price - price_5_days_ago) / price_5_days_ago
            if short_term_gain > 0.30:  # 调整为>30%排除，移除<5%条件
                return True, f"五日内涨幅{short_term_gain:.1%}超过30%，排除追高风险", {
                    'quality_score': quality_score,
                    'cross_stage': cross_stage,
                    'filter_type': 'price_increase'
                }
...
def worker(args):
    ...
    try:
        ...
        elif STRATEGY_TO_RUN == 'REVERSED_SHORT':  # 新添加的策略处理
            return _process_reversed_short_strategy(df, result_base, stock_code_full)
...
def _process_reversed_short_strategy(df, result_base, stock_code_full):
    """
    处理新策略REVERSED_SHORT：基于做空策略的反转，用于筛选做多候选
    注意：实际需在strategies.py中实现apply_reversed_short，这里假设已实现或添加逻辑
    """
    try:
        # 假设strategies.apply_reversed_short已实现，如果未实现，可在此处添加临时实现
        # 临时实现示例（如果strategies未有）：
        # signal_series = apply_reversed_short_temp(df)  # 如果需要
        signal_series = strategies.apply_reversed_short(df)  # 假设已添加至strategies
        
        if signal_series is not None and signal_series.iloc[-1]:
            # 可以添加特定过滤器，如果需要（类似其他策略）
            # 这里简单通过，无额外过滤
            backtest_stats = calculate_backtest_stats_fast(df, signal_series)
            result_base.update({
        ...
        logger.error(f"处理REVERSED_SHORT策略失败 {stock_code_full}: {e}")
        return None

# 如果strategies.py未实现apply_reversed_short，可添加以下临时函数
def apply_reversed_short_temp(df):
    """
    临时实现：反转做空策略信号，用于筛选做多候选股票
    - 积极信号：价格突破MA20且放量, MACD底背离, RSI超卖金叉
    - 至少2个信号满足
    """
    if len(df) < 30:
    ...
    def detect_bottom_divergence(df_recent):
        price_lows = df_recent['close'].rolling(5).min()
        macd_lows = df_recent['macd'].rolling(5).min()
        return (price_lows.iloc[-1] < price_lows.iloc[-2]) and (macd_lows.iloc[-1] > macd_lows.iloc[-2])
    ...
    for i in range(30, len(df)):
        ...
        positive_signals = []
        if current['close'] > current['ma20'] and current['volume'] > 1.2 * prev['volume']:
            positive_signals.append('break_above_ma20')
        
        df_recent = df.iloc[i-29:i+1]
        if detect_bottom_divergence(df_recent):
            positive_signals.append('macd_bottom_divergence')
        
        if current['rsi'] < 30 and current['rsi'] > prev['rsi']:
            positive_signals.append('rsi_oversold')
