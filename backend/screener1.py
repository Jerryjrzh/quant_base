...
# === 新增策略逻辑 START ===
# 因为您未提供 strategies.py 文件，我将新策略的逻辑直接添加到这里
# 在实际应用中，您应该将此函数移动到 strategies.py 文件中
def apply_value_reversal_strategy(df):
    """
    价值反转策略 (VALUE_REVERSAL)
    寻找满足以下条件中至少两个的股票：
    1. MACD底背离：价格创新低，但MACD未创新低。
    2. RSI超卖区启动：RSI从30以下的超卖区回升。
    3. 放量突破MA20：股价上穿20日均线，且成交量放大。
    """
    ...
    try:
        ...
        # 条件1：MACD底背离检查 (简化版)
        # 寻找过去60天内的最低点
        price_low_idx = df['low'][last_idx-59:last_idx+1].idxmin()
        if last_idx - df.index.get_loc(price_low_idx) < 10: # 如果最低点在最近10天内
            # 价格创新低，但DIF没有创新低
            if df['low'].iloc[last_idx] < price_low_idx and df['dif'].iloc[last_idx] > df['dif'].loc[price_low_idx]:
                 conditions_met += 1

        # 条件2：RSI超卖区启动
        if df['rsi'].iloc[last_idx-1] < 35 and df['rsi'].iloc[last_idx] > df['rsi'].iloc[last_idx-1]:
            conditions_met += 1
            
        # 条件3：放量突破MA20
        # 昨日在均线下方，今日在均线上方
        if (df['close'].iloc[last_idx-1] < df['ma20'].iloc[last_idx-1] and
            df['close'].iloc[last_idx] > df['ma20'].iloc[last_idx] and
            df['volume'].iloc[last_idx] > df['volume_ma20'].iloc[last_idx] * 1.5): # 成交量为20日均量的1.5倍
            conditions_met += 1

        # 如果满足至少2个条件，则产生信号
        if conditions_met >= 2:
            signal_series.iloc[last_idx] = True
...
def _process_triple_cross_strategy(df, result_base, stock_code_full):
    ...
    try:
        ...
        if signal_series is not None and signal_series.iloc[-1]:
            # === MODIFICATION START ===
            # 根据您的要求，暂时注释掉增强过滤器，因为它可能过于严格导致无法选出股票。
            ...
            # 由于禁用了上面的过滤器，我们直接使用基本回测数据
            backtest_stats = calculate_backtest_stats_fast(df, signal_series)
            result_base.update({
                'quality_score': 'N/A', # 因为过滤器被禁用
                'cross_stage': 'N/A', # 因为过滤器被禁用
                'filter_status': 'disabled', # 标记过滤器状态
                **backtest_stats
            })
            # === MODIFICATION END ===
            return result_base
