...
class UniversalScreener:
    ...
    def run_screening(self, strategy_ids: List[str], max_workers: Optional[int] = None) -> List[HighQualityResult]:
        ...
        if high_quality_results:
            ...
            # 保存深度扫描结果到文件，按分数排序
            self._save_deep_scan_results(high_quality_results)
    ...
    def _save_deep_scan_results(self, results: List[HighQualityResult]):
        ...
        try:
            ...
            # 创建结果目录
            result_dir = os.path.join(os.path.dirname(__file__), '..', 'data', 'result', 'ENHANCED_ANALYSIS')
            ...
            formatted_results = {}
            for result in results:
                formatted_results[result.stock_code] = {
                    'overall_score': {
                        'total_score': result.confluence_score,
                        'grade': result.quality_grade
                    },
                    'recommendation': {
                        'action': 'BUY' if result.confluence_score >= 85 else 'WATCH',
                        'confidence': result.confidence
                    },
                    'basic_analysis': {
                        'current_price': result.current_price,
                        'price_change_30d': 0,  # 可以后续计算
                        'volatility': 0,  # 可以后续计算
                        'signal_count': 1
                    },
                    'market_phase': result.market_phase,
                    'signal_type': result.signal_type,
                    'signal_date': result.date.isoformat() if hasattr(result.date, 'isoformat') else str(result.date),
                    'stock_name': result.stock_name
                }
            ...
            logger.info(f"📄 深度扫描结果已保存到: {filepath}")
            
        except Exception as e:
            logger.error(f"保存深度扫描结果失败: {e}")