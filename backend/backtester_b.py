...
def _generate_forward_advice(df: pd.DataFrame, backtest_results: dict) -> dict:
    """
    【已重构】基于最新的数据和历史回测的最优系数，生成前瞻性的交易建议。
    新版逻辑增加了对“长期下跌后底部企稳”形态的识别。
    """
    try:
        # --- 1. 数据准备 ---
        latest = df.iloc[-1]
        ...
        is_long_term_decline = current_price < high_250d * 0.6  # 当前价比年内高点跌去40%以上
        ...
        is_consolidating = consolidation_range < 0.15  # 30日振幅小于15%
        
        # c. 是否已从近期低点反弹？
        is_bouncing_from_low = current_price > low_30d * 1.05 # 从30日低点反弹超过5%
        ...
        # 规则1：底部企稳形态 (最高优先级)
        if is_long_term_decline and is_consolidating and is_bouncing_from_low:
            action = 'BUY'
            confidence = 0.75 # 给予较高的置信度
            reasons.append("形态识别：该股经历长期下跌后，近期处于横盘震荡区间，有底部企稳迹象。")
            reasons.append(f"价格已从30日低点 ¥{low_30d:.2f} 反弹，下跌风险相对较小。")
        
        # 规则2：常规技术指标分析 (作为辅助或在无特殊形态时使用)
        # 只有在没有触发底部形态时，才进行常规判断
        if action == 'HOLD': 
            if latest['rsi6'] < 30:
                action = 'BUY'
                confidence = 0.70
                reasons.append(f"技术指标：RSI(6)为{latest['rsi6']:.1f}，进入短期超卖区，存在反弹机会。")
            elif latest['close'] < latest['ma60']:
                # 这条规则现在只在没有更好机会时才触发
                action = 'AVOID'
                confidence = 0.50 # 维持原置信度
                reasons.append(f"趋势判断：价格位于长期均线MA60(¥{latest['ma60']:.2f})下方，主要趋势仍偏弱。")
            else:
