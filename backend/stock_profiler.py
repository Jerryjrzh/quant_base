...
class StockProfiler:
    ...
    def create_stock_profile(self, stock_code: str, method: str = 'differential_evolution', is_worker: bool = False) -> bool:
        ...
        """
        if not is_worker:
            self.logger.info(f"开始为 {stock_code} 生成参数画像（智能模式）")

        try:
            # --- 模板匹配逻辑 ---
            if self.templates:
                # 简化逻辑：先用第一个模板尝试
                # 生产环境可以根据股票特征（如行业）动态选择模板
                template_id = "0" 
                template_params = self.templates.get(template_id)

                if template_params:
                    df_full = data_handler.get_full_data_with_indicators(stock_code)
                    if df_full is None or len(df_full) < 550:
                        self.logger.warning(f"{stock_code} 数据不足，无法生成画像")
                        return False

                    validation_score = self._validate_parameters(df_full, template_params)
                    VALIDATION_THRESHOLD = 0.6 # 可接受的性能门槛

                    if validation_score >= VALIDATION_THRESHOLD:
                        self.logger.info(f"✅ {stock_code} [快速路径] 成功匹配模板 {template_id}，分数: {validation_score:.3f}")
                        profile_data = {
                            'optimized_params': json.dumps({**template_params, 'validation_score': validation_score, 'source': f'template_{template_id}'}),
                            'optimization_method': 'template_matching',
                            'optimization_date': datetime.now().isoformat()
                        ...
                        self.logger.info(f"{stock_code} 模板匹配失败 (分数: {validation_score:.3f})，尝试快速优化...")
                        
                        # --- 快速优化逻辑 ---
                        optimized_params = self._optimize_with_minimize(df_full.tail(550), initial_guess=list(template_params.values()))
                        if optimized_params and optimized_params.get('optimization_success'):
                            final_params = optimized_params
                            final_params['validation_score'] = self._validate_parameters(df_full, final_params)
                            
                            self.logger.info(f"✅ {stock_code} [快速路径] 快速优化成功，分数: {final_params['validation_score']:.3f}")
                            profile_data = {
                                'optimized_params': json.dumps(final_params),
                                'optimization_method': 'template_L-BFGS-B',
                                'optimization_date': datetime.now().isoformat()
                            }
                            return self.pool_manager.update_stock_profile(stock_code, profile_data)
            
            # --- 完整优化逻辑 (作为回退) ---
            self.logger.info(f"{stock_code} [完整路径] 快速路径失败，执行完整差分进化算法...")
            df = data_handler.get_full_data_with_indicators(stock_code)
            if df is None or len(df) < 550:
                self.logger.warning(f"{stock_code} 数据不足 (少于550天)，无法生成画像")
                return False
            
            recent_df = df.tail(550)
            ...
            if success:
                self.logger.info(f"✅ {stock_code} [完整路径] 成功生成参数画像，验证分数: {validation_score:.3f}")
            return success
    ...
    def run_profiling_for_pool_old(self, limit: Optional[int] = None, use_multiprocessing: bool = True) -> Dict[str, int]:
        self.logger.info(f"开始为核心观察池生成参数画像 ({'多进程' if use_multiprocessing else '单进程'}模式)")
         # 如果未提供股票列表，则从数据库中获取核心池
    ...
    def run_profiling_for_pool(self, stock_codes: Optional[List[str]] = None, limit: Optional[int] = None, use_multiprocessing: bool = True, max_workers: Optional[int] = None) -> Dict[str, int]:
        """Run profiling for a list of stocks or the core pool, with optional multiprocessing."""
        self.logger.info(f"开始为核心观察池生成参数画像 ({'多进程' if use_multiprocessing else '单进程'}模式)")
        
        # If stock_codes is not provided, fetch from core pool
        if stock_codes is None:
            core_pool = self.pool_manager.get_core_pool(limit=limit)
            stock_codes = [stock['stock_code'] for stock in core_pool]
        else:
        ...
        if use_multiprocessing and len(stock_codes) > 1:
            ...
            for i, stock_code in enumerate(stock_codes, 1):
                self.logger.info(f"处理进度 [{i}/{len(stock_codes)}]: {stock_code}")
                if self.create_stock_profile(stock_code): 
                    results['success'] += 1
                else: 
                    results['failed'] += 1
