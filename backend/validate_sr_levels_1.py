#!/usr/bin/env python3
"""
支撑位与阻力位 (Support/Resistance) 分板块有效性回测验证工具
"""
...
def validate_support_resistance(sample_size=0):
    ...
    for idx, row in df_csv.iterrows():
        ...
        dynamic_tolerance = float(market_profile['sr_tolerance'])
        board_name = market_profile['board_type']
        
        print(f"[{idx:04d}/{len(df_csv)}] 分析 {stock} ({board_name}) @ {entry_date} ... ", end="")
        ...
        try:
            ...
            if 'error' in analysis:
                print(f"❌ 提取失败: {analysis['error'][:50]}")
                continue
            ...
            # 安全获取支撑阻力位
            support_level = advice.get('support_level')
            support_level = float(support_level) if support_level else None
            
            resistance_level = advice.get('resistance_level')
            resistance_level = float(resistance_level) if resistance_level else None
            
            # 强制转换为浮点数，防止 CSV 格式异常
            future_min = float(row['future_min_low'])
            future_max = float(row['future_max_high'])
            ...
            results.append({
                ...
                'dynamic_tol': dynamic_tolerance,
                'current_price': current_price,
                'future_min': future_min,
                'future_max': future_max,
                
                '支撑位': support_level,
                'supp_dist_pct': round(supp_dist, 4) if supp_dist is not None else None,
                'supp_tested': supp_tested,
                'supp_held': supp_held,
                'supp_broken': supp_broken,
                
                '阻力位': resistance_level,
                'res_dist_pct': round(res_dist, 4) if res_dist is not None else None,
            ...
            print(f"❌ 内部异常: {str(e)}")
            traceback.print_exc()  # 把报错堆栈打出来方便定位
            continue

    # 结果安全性校验
    if not results:
        print("\n⚠️ 所有的样本均计算失败，没有生成有效结果！请查看上方的 ❌ 内部异常 报错日志。")
        return None
    ...
    print(f"  平均下方距离 : {avg_supp_dist:.2f}%")
    print(f"  实际被测试次 : {total_supp_tested} 次")
    if total_supp_tested > 0:
        print(f"  🟢 支撑有效率 : {supp_held_rate:.1f}%")
        print(f"  🔴 破位失效占比: {supp_broken_rate:.1f}%")
        
    print("-" * 80)
    print("📈 阻力位 (Resistance) 表现:")
    print(f"  平均上方距离 : {avg_res_dist:.2f}%")
    print(f"  实际被测试次 : {total_res_tested} 次")
    if total_res_tested > 0:
        print(f"  🔴 阻力压制率 : {res_held_rate:.1f}%")
        print(f"  🟢 强势突破率 : {res_broken_rate:.1f}%")
    print("="*80)
    
    output_path = "/home/hypnosis/data/quant_base/data/result/Calendar_Backtest/sr_validation_results_full.csv"
