...
def _optimization_worker(args):
    """优化任务的工作函数，用于多进程优化"""
    method, df, param_bounds, default_params = args
    profiler = StockProfiler()  # 创建新的实例以避免进程间冲突
    if method == 'differential_evolution':
        return profiler._optimize_with_differential_evolution(df)
    elif method == 'minimize':
        return profiler._optimize_with_minimize(df)
    return None

def _validation_worker(args):
    """验证任务的工作函数，用于多进程验证"""
    df, params = args
    profiler = StockProfiler()  # 创建新的实例
    return profiler._validate_parameters(df, params)
    ...
    def create_stock_profile(self, stock_code: str, method: str = 'differential_evolution', is_worker: bool = False) -> bool:
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=2) as executor:
                tasks = [
                    ('differential_evolution', recent_df, self.param_bounds, self.default_params),
                    ('minimize', recent_df, self.param_bounds, self.default_params)
                ]
                futures = [executor.submit(_optimization_worker, task) for task in tasks]
                ...
                for future in as_completed(futures):
                    ...
                    if result and result.get('optimization_success') and result.get('optimization_error', float('inf')) < min_error:
                        optimal_params_dict = result
                        min_error = result.get('optimization_error', float('inf'))
            ...
            with ProcessPoolExecutor(max_workers=1) as executor:
                future = executor.submit(_validation_worker, (df, optimal_params_dict))
                validation_score = future.result()
