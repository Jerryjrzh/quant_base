        ...
        for idx, row in future_df.iterrows():
            ...
            if trade_status == "未成交":
                ...
                if row['low'] <= dynamic_trigger_buy:
                    # 🚀 【补丁2：缩量双底免检权】
                    # 如果当天是极度缩量洗盘（量比小于 0.6），说明抛压枯竭，无需强行要求拉升 0.5% 来确认
                    # 或者，只要拉升超过了 0.5%，也算确认
                    is_shrink_bottom = vol_ratio < 0.60
                    is_strong_rebound = row['close'] >= row['low'] * 1.005
                    
                    if is_strong_rebound or is_shrink_bottom:  
                        trade_status = "持仓中"
                        entry_date = current_date_str
                        # 如果是免检权进入的，买入价就按最低价往上稍微浮动一点点算
                        actual_entry_slip = 1.005 if is_strong_rebound else 1.002
                        entry_price = min(dynamic_trigger_buy, row['low'] * actual_entry_slip) 
