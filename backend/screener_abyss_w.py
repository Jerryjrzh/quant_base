...
CHART_DIR = os.path.join(RESULT_DIR, 'charts_weekly') # 修改图表目录名
...
logger = logging.getLogger('abyss_screener_weekly')
...
class AbyssBottomingStrategyV2:
    ...
    def _analyze_volume_shrinkage(self, df):
        p_volume = self.config_weekly.get('volume_analysis', {})
        recent_periods = p_volume.get('volume_analysis_periods', 6) # 30天 -> 6周
        mid_term_periods = 18 # 90天 -> 18周
        if len(df) < recent_periods + mid_term_periods: return False, {}

        mid_term_df = df.iloc[-recent_periods-mid_term_periods:-recent_periods]
        ...
        is_shrunk = shrink_ratio < p_volume.get('volume_shrink_threshold', 0.7)
    ...
    def _check_stage0_decline(self, df):
        ...
        periods = p_decline.get('long_term_periods', 80) # 400天 -> 80周
        ...
        drop_ok = drop_pct > p_decline.get('min_drop_percent', 0.40)
    ...
    def _check_stage1_hibernation(self, df):
        p_washout = self.config_weekly.get('washout_phase', {})
        p_hibernation = self.config_weekly.get('hibernation_phase', {})
        w_periods = p_washout.get('washout_periods', 3) # 15天 -> 3周
        h_periods = p_hibernation.get('hibernation_periods', 8) # 40天 -> 8周
        if len(df) < w_periods + h_periods: return False, {}
        hib_df = df.iloc[-w_periods-h_periods:-w_periods]
        ...
        volatility_ok = volatility < p_hibernation.get('hibernation_volatility_max', 0.4)
    ...
    def _check_stage2_washout(self, df, hibernation_info):
        p_washout = self.config_weekly.get('washout_phase', {})
        w_periods = p_washout.get('washout_periods', 3)
        if len(df) < w_periods: return False, {}
        washout_df = df.tail(w_periods)
        support, pit_low = hibernation_info['support'], washout_df['low'].min()
        broken_ok = pit_low < support * p_washout.get('washout_break_threshold', 0.95)
        pit_days_df = washout_df[washout_df['low'] < support]
        if len(pit_days_df) < p_washout.get('min_pit_periods', 1): return False, {} # 周线级别，至少有1周破位即可
        pit_avg_vol, hib_avg_vol = pit_days_df['volume'].mean(), hibernation_info['avg_volume']
        if hib_avg_vol == 0: return False, {}
        shrink_ratio = pit_avg_vol / hib_avg_vol
        shrink_ok = shrink_ratio < p_washout.get('washout_volume_shrink_ratio', 0.85)
    ...
    def _check_stage3_liftoff(self, df, washout_info):
        p_liftoff = self.config_weekly.get('liftoff_phase', {})
        ...
        near_bottom = rise_from_bottom < p_liftoff.get('max_rise_from_bottom', 0.18)
        if washout_info['pit_avg_volume'] == 0: return False, {}
        volume_increase_ratio = latest['volume'] / washout_info['pit_avg_volume']
        volume_confirming = volume_increase_ratio > p_liftoff.get('liftoff_volume_increase_ratio', 1.15)
    ...
    def apply_strategy(self, df):
        try:
            ...
            signal_series.iloc[-1] = 'ABYSS_BUY_WEEKLY'
            signal_details = { 'signal_state': 'FULL_PATTERN_CONFIRMED_WEEKLY', **stage0_info, **stage1_info, **stage2_info, **stage3_info }
...
def visualize_signal(df_weekly, stock_code, signal_date_str, details):
    ...
    try:
        ...
        # 修正：周线图lookback周期
        start_date = signal_date - pd.Timedelta(weeks=104) # 显示约2年的周线
        df_chart = df_chart.loc[start_date:]
        if df_chart.empty: return

        title = (f"WEEKLY CHART: {stock_code} on {signal_date_str} - ABYSS_BUY\n"
                 f"Rise from Pit: {details.get('rise_from_bottom', 0):.1%}, "
                 f"Vol Ratio: {details.get('volume_increase_ratio', 0):.1f}x")
        ...
        # 修正：找到信号日期所在周的索引
        signal_week_index = df_chart.index[df_chart.index.weekofyear == signal_date.weekofyear]
        ...
        if 'support' in details and 'pit_low' in details:
            axes[0].axhline(y=details.get('support'), color='orange', linestyle='--', alpha=0.7, label='Hibernation Support')
            axes[0].axhline(y=details.get('pit_low'), color='red', linestyle=':', alpha=0.7, label='Washout Pit Low')
            axes[0].legend()
    ...
    try:
        ...
        if signal_series is not None and details is not None and signal_series.iloc[-1] == 'ABYSS_BUY_WEEKLY':
            result = {
                ...
                'date': df_weekly.index[-1].strftime('%Y-%m-%d'), # 使用周五作为日期
            ...
            logger.info(f"发现高质量周线信号: {stock_code}")
            return result
        return None
    except Exception as e:
...
def main():
    ...
    logger.info("===== 开始执行深渊筑底策略筛选 (周线分析版) =====")
    ...
    output_file = os.path.join(RESULT_DIR, f'abyss_signals_weekly_{DATE}.json')
