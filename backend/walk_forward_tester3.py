...
def generate_strategy_signals(df, strategy_name):
    ...
    df_index = data_loader.get_daily_data("sh000852.day 路径")
    regime = MarketRegimeDetector().evaluate_regime(df_index, EVAL_DATE)
    if strategy_name == 'ADAPTIVE_MA_SUPPORT':
        ...
        # 根据regime动态调整 trigger_buy 和 stop_loss
        if regime['risk_score'] > 70:
            trigger_buy *= regime['discount']
            stop_loss *= regime['discount'] * 0.98   # 止损更紧
