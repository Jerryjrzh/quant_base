...
# 修改点：引入ProcessPoolExecutor以支持多进程，同时保留ThreadPoolExecutor以备其他用途
...
def _deep_scan_stocks_fallback(stock_codes, use_optimized_params=True, max_workers=32):
    """标准多进程分析多只股票（备用方法）"""
    # 修改点：将此函数改造为使用多进程
    print(f"🚀 多进程批量分析 {len(stock_codes)} 只股票 (进程数: {max_workers})")
    
    results = {}
    completed_count = 0
    
    # 修改点：使用 ProcessPoolExecutor 替代 ThreadPoolExecutor
...
def deep_scan_stocks(stock_codes, use_optimized_params=True, max_workers=32):
    ...
    try:
        ...
        # 如果性能优化模块不可用，使用原始方法（修改点：现在回退到多进程方法）
        print(f"⚠️ 性能优化模块导入失败: {e}，使用标准多进程方法")
