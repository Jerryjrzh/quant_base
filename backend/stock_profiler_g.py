...
def _profiling_worker_process(args):
    """独立的、可被多进程调用的工作函数"""
    stock_code, db_path = args
    # 在新进程中，需要重新创建实例
    profiler = StockProfiler(db_path)
    return profiler.create_stock_profile(stock_code, is_worker=True)
...
class StockProfiler:
 ...
 # --- 核心修改：create_stock_profile 改为串行执行，移除内部的ProcessPoolExecutor ---
    ...
    def _validate_parameters(self, df: pd.DataFrame, params: Dict[str, Any]) -> float:
     ...
     # --- 核心修改：run_profiling_for_pool 成为唯一的并行任务分发者 ---
    def run_profiling_for_pool(self, limit: Optional[int] = None, use_multiprocessing: bool = True) -> Dict[str, int]:
        ...
        if use_multiprocessing and len(stock_codes) > 1:
            # 准备传递给工作函数的参数
            tasks = [(sc, self.pool_manager.db_path) for sc in stock_codes]
