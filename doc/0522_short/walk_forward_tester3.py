...
def _calculate_priority_score(df, backtest_stats, features=None):
    """
    【升级完全体】综合优先级评分体系（0-100分制）
    核心哲学：安全第一（周线底安全垫），收益第二（阻力位预期空间），历史自证第三。
    """
    try:
        score = 0.0
        ...
        # ---------------------------------------------------------
        # 维度 1：历史胜率与利润基准（基础分，占比 40%）
        # ---------------------------------------------------------
        win_rate_str = backtest_stats.get('win_rate', '0.0%').replace('%', '')
        win_rate = float(win_rate_str) / 100.0
        score += win_rate * 20  # 胜率权重 20分

        profit_str = backtest_stats.get('avg_max_profit', '0.0%').replace('%', '')
        avg_profit = float(profit_str) / 100.0
        score += min(avg_profit / 0.4, 1.0) * 20  # 历史均益权重 20分

        # ---------------------------------------------------------
        # 维度 2：📐 压力位预期收益空间增益（占比 30%）
        # ---------------------------------------------------------
        # 读取上游抛出的近期阻力位（压力位）
        recent_resistance = features.get('recent_resistance', current_price * 1.15)
        # 计算当前价格距离压力位的潜在预期反弹空间
        expected_upside = (recent_resistance - current_price) / current_price
        
        if expected_upside > 0:
            # 预期收益空间越大，期望得分越高（15% 以上的反弹空间拿满 30 分增益）
            score += min(expected_upside / 0.15, 1.0) * 30
            
        # ---------------------------------------------------------
        # 维度 3：🛡️ 周线大底安全垫边际（占比 20%）
        # ---------------------------------------------------------
        # 读取周线历史大底
        weekly_floor_low = features.get('weekly_floor_low', current_price * 0.90)
        # 计算当前价格距离周线历史底的距离
        distance_to_floor = (current_price - weekly_floor_low) / current_price
        
        # 离周线大底越近，安全垫越厚
        if distance_to_floor <= 0.03:
            score += 20  # 紧贴周线底，防空洞级别安全，加满20分
        elif distance_to_floor <= 0.06:
            score += 12  # 相对安全
        elif distance_to_floor > 0.12:
            score -= 15  # 距离周线底部过远，属于虚浮的空中楼阁，惩罚扣分
        ...
        if len(df) >= 20:
            ...
            if vol_20 > 0:
                vol_ratio = min(vol_5 / vol_20, 2.5)
                score += (vol_ratio / 2.5) * 10

        # 将预期期望收益（反弹空间空间比例）动态更新回统计卡中，方便在出线卡中直接观察
        backtest_stats['expected_upside_pnl'] = f"{expected_upside:.1%}"
        backtest_stats['weekly_floor_low'] = weekly_floor_low
...
def generate_strategy_signals(df, strategy_name):
    ...
    if strategy_name == 'ADAPTIVE_MA_SUPPORT':
        ...
        short_term_drop = (current_price - recent_5d_high) / recent_5d_high
        
        # 2. 计算当天的量能状态 (判断抛压是否释放完毕)
        vol_ma20 = df['volume'].rolling(20).mean().iloc[-1]
        curr_vol_ratio = df['volume'].iloc[-1] / vol_ma20 if vol_ma20 > 0 else 1.0
        ...
        for idx, row in future_df.iterrows():
            ...
            if trade_status == "未成交":
                ...
                if market_crash_today:
                    ...
                    # ✅ 常规交易日：回归原汁原味的经典九八防线，绝不乱漂移
                    dynamic_trigger_buy = trigger_buy * 0.98
                    dynamic_stop_loss = stop_loss * 0.98
                ...
                if row['low'] <= dynamic_trigger_buy:
                    # 确认底部有资金承接（拉回比例严格按你对账单赚钱的 0.5% 设定）
                    if row['close'] >= row['low'] * 1.005:  
                        trade_status = "持仓中"
                        entry_date = current_date_str
                        entry_price = min(dynamic_trigger_buy, row['low'] * 1.005) 
                        
                        # 🔒 核心锁死：买入瞬间，将当前计算出的止损位锁死到实际执行卡中
                        actual_stop_loss = dynamic_stop_loss
                        
                        # 买入当天如果遭遇极端的极端向下贯穿，直接强制止损
                ...
                # 常规阶梯止盈
                current_target_profit = TARGET_PROFIT if holding_days <= 6 else DECAY_PROFIT
