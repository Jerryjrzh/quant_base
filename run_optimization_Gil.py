...
# 创建一个全局的打印锁，防止多线程输出时内容交错
print_lock = threading.Lock()

def optimize_stock_parameters(stock_code, optimization_target='win_rate'):
    """优化指定股票的参数 (线程安全打印)"""
    with print_lock:
        print(f"🎯 [{stock_code}] 开始优化交易参数...")
        print(f"📊 [{stock_code}] 优化目标: {optimization_target}")
        print("=" * 60)
    ...
    try:
        ...
        if signal_count < 5:
            with print_lock:
                print(f"⚠️ [{stock_code}] 信号数量({signal_count})较少，优化结果可能不够可靠")
...
def batch_optimize_stocks(stock_codes, optimization_target='win_rate', max_workers=32):
    """批量优化多只股票 (多线程)"""
    total_jobs = len(stock_codes)
    print(f"🚀 批量优化 {total_jobs} 只股票 (使用最多 {max_workers} 个线程)")
...
def main():
    ...
    if sys.argv[1] == 'batch':
        ...
        optimization_target = 'win_rate' # 在批量模式下可以硬编码一个默认目标，或使其可配置
        # 自动确定线程数，或使用默认值8
        max_workers = min(32, os.cpu_count() + 4) if os.cpu_count() else 16
        batch_optimize_stocks(stock_codes, optimization_target, max_workers=max_workers)
